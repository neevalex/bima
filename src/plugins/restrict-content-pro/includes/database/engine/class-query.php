...
class Query extends Base {
	 ...
	 * @var   string
	 */
	protected $table_schema = '\\RCP\\Database\\Schemas\\Base';
	 ...
	 *
	 * I.E. `\\RCP\\Database\\Row` or `\\RCP\\Database\\Rows\\Customer`
	 *
	 ...
	 * @var   mixed
	 */
	protected $item_shape = '\\RCP\\Database\\Row';
	...
	private function set_item_shape() {
		if ( empty( $this->item_shape ) || ! class_exists( $this->item_shape ) ) {
			$this->item_shape = 'RCP\\Database\\Row';
		}
	}
	...
	private function parse_query( $query = array() ) {
		 ...
		 * @since 3.0
		 *
		 * @param \RCP\Database\Query &$this The \RCP\Database\Query instance (passed by reference).
		 */
	...
	private function parse_where() {
		...
		foreach ( $this->columns as $column ) {
			...
			// Is NULL
			if ( null === $this->query_vars[ $column->name ] ) {
				// Add to where array
				$where[ $column->name ] = "{$this->table_alias}.{$column->name} IS NULL";
			}
			...
			if ( true === $column->not_in ) {
				...
				if ( isset( $this->query_vars[ $where_id ] ) && is_array( $this->query_vars[ $where_id ] ) ) {
					...
					// Convert single item arrays to literal column comparisonsurn:uuid:c864c5f5-bacc-460e-8859-baa10b31a759
					if ( 1 === count( $this->query_vars[ $where_id ] ) ) {
						$column_value = reset( $this->query_vars[ $where_id ] );
						$statement    = "{$this->table_alias}.{$column->name} != %s";
	...
	private function shape_items( $items = array() ) {
		 ...
		 *
		 * Looks like `rcp_get_memberships`
		 *
		 * @since 3.0
		 *
	...
	public function get_results( $cols = array(), $where_cols = array(), $limit = 25, $offset = null, $output = OBJECT ) {
		...
		if ( ! empty( $where_cols ) ) {
			...
			foreach ( $where_cols as $column => $compare ) {
				...
				// Basic WHERE clause.
				if ( ! is_array( $compare ) ) {
					$query .= $this->get_db()->prepare( " AND {$this->table_alias}.{$column} = %s ", $compare );
