...
function rcp_add_membership( $data = array() ) {
	...
	$membership_level = rcp_get_subscription_details( $data['object_id'] );
	...
	if ( false === $data['initial_amount'] ) {
		$data['initial_amount'] = $membership_level->fee + $membership_level->price;
	}
	if ( false === $data['recurring_amount'] ) {
		$data['recurring_amount'] = $membership_level->price;
	}
